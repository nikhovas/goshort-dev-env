...
Vagrant.configure("2") do |config|
  ...
  config.vm.synced_folder "tools/saltstack/salt", "/srv/salt"
  config.vm.synced_folder "tools/saltstack/pillar", "/srv/pillar/"
  ...
  config.vm.provision :salt do |salt|
  	...
  	salt.minion_config = "tools/saltstack/minion-config"
    salt.verbose = true
